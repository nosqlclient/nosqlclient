...
const proceedMapReduceExecution = (selectedCollection, map, reduce, options, sessionId) => {
  ...
  const result = Async.runSync((done) => {
    try {
      ...
      collection.mapReduce(map, reduce, options, (firstError, resultCollection) => {
        if (firstError) {
...
const proceedExecutionStepByStep = (entry, last, done, execution) => {
...
const proceedQueryExecution = (selectedCollection, methodArray, sessionId, removeCollectionTopology) => {
...
Meteor.methods({
  saveFindResult(selectedCollection, updateObjects, deleteObjectIds, addedObjects, sessionId) {
    Object.entries(updateObjects).forEach(obj => proceedQueryExecution(selectedCollection, [{ updateOne: [{ _id: obj._id }, obj, {}] }], sessionId));

    if (deleteObjectIds.length > 0) proceedQueryExecution(selectedCollection, [{ deleteMany: [{ _id: { $in: deleteObjectIds } }] }], sessionId);
    if (addedObjects.length > 0) proceedQueryExecution(selectedCollection, [{ insertMany: [addedObjects] }], sessionId);
  },
