...
Helper.prototype = {
  ...
  getDistinctKeysForAutoComplete(selectedCollection) {
    ...
    const countToTake = isNaN(parseInt(settings.autoCompleteSamplesCount, 10)) ? 50 : parseInt(settings.autoCompleteSamplesCount, 10);
...
(function () {
  ...
  Template.registerHelper('getConfiguredAutoCompletionKey', () => Settings.findOne().autoCompleteShortcut || 'Ctrl-Space');
  ...
  Template.registerHelper('getSelectedCollection', () => Session.get(helper.strSessionSelectedCollection));

  Template.registerHelper('isConnected', () => (Session.get(helper.strSessionCollectionNames) != undefined));
}());
