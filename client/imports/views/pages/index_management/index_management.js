...
const populateTableData = function (indexInfo, stats, indexStats) {
  const result = [];
  for (const obj of indexInfo.result) {
    const index = {
      name: obj.name,
      asc_fields: [],
      desc_fields: [],
    ...
    if (indexStats && indexStats.result) {
      for (const indexStat of indexStats.result) {
...
Template.indexManagement.onRendered(function () {
  ...
  $('#addIndexModal').on('shown.bs.modal', () => {
    prepareFormForView();
  });
  $('#viewRawModal').on('shown.bs.modal', () => {
    initialize();
  });
