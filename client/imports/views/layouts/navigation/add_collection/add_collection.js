...
const prepareFormAsView = function () {
  ...
  $.each(Session.get(Helper.strSessionCollectionNames), (index, value) => {
    cmb.append($('<option></option>')
...
const gatherOptions = function () {
  ...
  const collationVal = Helper.getCodeMirrorValue($('#divCollationAddCollection'));
  if (collationVal) {
  ...
  const storageEnginveVal = Helper.getCodeMirrorValue($('#divStorageEngine'));
  ...
  const validatorVal = Helper.getCodeMirrorValue($('#divValidatorAddCollection'));
  if (validatorVal) {
...
const initializeOptions = function () {
  ...
  $.each(Helper.sortObjectByKey(Enums.ADD_COLLECTION_OPTIONS), (key, value) => {
    cmb.append($('<option></option>')
      .attr('value', key)
      .text(value));
  });
